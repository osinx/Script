const $ = new Env("2ublu helper");

const blackList = [
  'http://47.97.243.214/',
  '/checkRoadNetUpdate?',
  '/checkUpdateByWhiteList',
  '/monitor/upBehavior',
  '/saveSplashFrequencyStatistics',
  '/getAppSkin',
  '/device?',
  '/dayActiveUser?',
  '/reqHasNew'
...
function fixUrl(url) {
  ...
  if (url.indexOf('/downtrack2') > -1) {
    ...
    url = url.replace('p_terminalType=3', 'p_terminalType=2');
  }

  if (url.indexOf('&psign=') > -1) {
    url = url.replace(/&psign=[a-zA-Z0-9]+/, '');
