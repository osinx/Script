/*
# https://www.jietiandi.net/plugin.php?id=zqlj_sign
# 兼容 QX/qinglong

# 抓取cookie
^https?:\/\/jietiandi.net\/plugin.php\?id=zqlj_sign&sign= url script-request-header https://raw.githubusercontent.com/osinx/Script/main/task/jietiandi_cookie.js
...
const signinfo = {}
let VAL_signurl = $.getdata(KEY_signurl) || ($.isNode() ? process.env[KEY_signurl] : '')
let VAL_signheader = $.getdata(KEY_signheader) || ($.isNode() ? process.env[KEY_signheader] : '')
    ...
    return new Promise((resolve, reject) => {
        ...
        $.post(url, (error, response, data) => {
            try {
                ...
                $.log(`❌ ${cookieName} signapp - 签到失败: ${e}`)
                $.log(`❌ ${cookieName} signapp - response: ${JSON.stringify(response)}`)
                resolve()
            }
        })
    })
...
function showmsg() {
    let subTitle = ''
    let detail = ''
    $.log(`showmsg: ${JSON.stringify(signinfo)}`);
    // 签到结果
    if (gosign == true) {
